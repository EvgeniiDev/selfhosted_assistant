...
class TaskSkillService:
    """Skill-backed task extractor that returns the existing Task model."""
    ...
    def __init__(
        ...
        self.capability_spec = self.capability_registry.get("task")
        self.skill_service = SkillExecutionService(
            gateway=self.gateway,
            spec=self._build_skill_spec(self.capability_spec),
        )

    def create_task(self, user_message: str, current_time_text: str) -> Task | None:
    ...
    def _parse_task(self, response_content: str) -> Task | None:
        parsed = self._extract_json(response_content)
        if not parsed or parsed.get("type") != "task":
            return None
        ...
        try:
            ...
            calendar_logger.log_error(exc, "task_skill_service._parse_task")
            return None
    ...
    def _extract_json(self, content: str) -> dict[str, Any] | None:
        try:
            ...
            calendar_logger.log_error(exc, "task_skill_service._extract_json")
            return None
